...
public class RotateRightCommandTest {
	...
	public void testSingleRotate() {
		...
		Rover result = new RotateRightCommand().execute(testRobot);
	...
	public void testTwoRotate() {
		Grid testGrid = new Grid(4,4);
		Rover testRobot = new Rover(RoverOrientation.NORTH,testGrid,new Position(2,2));
		Rover result = new RotateRightCommand().execute(
				                   new RotateRightCommand().execute(testRobot));
	...
	public void testThreeRotate() {
		...
		Rover testRobot = new Rover(RoverOrientation.NORTH,testGrid,new Position(2,2));
		Rover result = new RotateRightCommand().execute(
				                   new RotateRightCommand().execute(
				                		   new RotateRightCommand().execute(testRobot) 
				                		   ));
